...
class NetworkAnalyzer:
    ...
    def convert_matrix_to_edgetable(self) -> pd.DataFrame:
        ...
        try:
            # Load similarity matrix - handle sourmash format
            wide_df = pd.read_csv(self.input_file)

            # This is sourmash format: first row is genome names, no row labels
            # We need to use the column names as BOTH row and column identifiers
            genome_names = wide_df.columns.tolist()

            logger.info("Detected sourmash format - fixing matrix structure")
            logger.info(f"Genome names (first 5): {genome_names[:5]}")

            # Set the genome names as the index (row labels)
            wide_df.index = genome_names
            ...
            logger.info(f"Source column type: {wide_df_melted['Source'].dtype}")
            logger.info(f"Target column type: {wide_df_melted['Target'].dtype}")
            logger.info(f"Value column type: {wide_df_melted['Value'].dtype}")
            logger.info(f"Sample data:\n{wide_df_melted.head()}")

            # Ensure Source and Target are strings and handle any NaN values
            wide_df_melted['Source'] = wide_df_melted['Source'].astype(str)
            wide_df_melted['Target'] = wide_df_melted['Target'].astype(str)

            # Remove any rows where Source or Target are 'nan' or empty
            wide_df_melted = wide_df_melted[
                (wide_df_melted['Source'] != 'nan') &
                (wide_df_melted['Target'] != 'nan') &
                (wide_df_melted['Source'] != '') &
                (wide_df_melted['Target'] != '')
            ]

            logger.info(f"Initial edgetable size: {len(wide_df_melted)} edges")

            # Handle self-loops
            if not self.keep_self_loops:
                wide_df_melted = wide_df_melted[wide_df_melted['Source'] != wide_df_melted['Target']]
                logger.info(f"After removing self-loops: {len(wide_df_melted)} edges")

            logger.info("Starting edge deduplication process...")

            # Remove duplicate edges (undirected network) - using vectorized approach
            logger.info("Sorting source-target pairs...")

            # Much faster vectorized approach instead of apply()
            source_vals = wide_df_melted['Source'].values
            target_vals = wide_df_melted['Target'].values

            # Create arrays for sorted pairs
            logger.info("Creating sorted edge pairs...")
            sorted_pairs = np.column_stack([
                np.minimum(source_vals, target_vals),
                np.maximum(source_vals, target_vals)
            ])

            # Create new dataframe with sorted pairs
            wide_df_sorted = pd.DataFrame({
                'Source': sorted_pairs[:, 0],
                'Target': sorted_pairs[:, 1],
                'Value': wide_df_melted['Value'].values
            })
            ...
            logger.info(f"After removing duplicates: {len(wide_df_unique)} edges")

            logger.info("Removing NaN values...")
            wide_df_unique = wide_df_unique.dropna()
            logger.info(f"Final edgetable size: {len(wide_df_unique)} edges")
    ...
    def find_optimal_threshold(self, communities_df: pd.DataFrame) -> Tuple[int, pd.DataFrame]:
        ...
        try:
            ...
            # Create and save elbow plot (separate try-catch for plotting)
            try:
                self._plot_elbow_detection(rotor, optimal_weight)
            except Exception as plot_error:
                logger.warning(f"Could not create elbow plot: {plot_error}")
                logger.info("Elbow detection was successful, only plotting failed")
            ...
            logger.error(f"Error in elbow detection algorithm: {e}")
            # Fallback to simple heuristic only if elbow detection itself fails
            ...
            logger.info(f"Fallback threshold selected: {optimal_weight}% similarity")
            return optimal_weight, auc_data
    ...
    def _plot_elbow_detection(self, rotor: Rotor, optimal_weight: int) -> None:
        ...
        try:
            ...
            # Fallback for older kneebow versions
            plt.figure(figsize=(10, 6))
            rotor.plot_elbow()
            plt.title(f'Elbow Detection - Optimal Weight: {optimal_weight}%', fontsize=14)
            plt.xlabel('Minimum Edge Weight (%)', fontsize=12)
            plt.ylabel('Cumulative AUC', fontsize=12)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Purgatorio - A community analysis tool for genome-genome networks",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Named after Dante's Purgatorio, representing the transformative space of co-evolution.
